...
%     % Comprimento do vetor h
%     M = length(h);
%     tolerance = 1e-4;  % Tolerância ajustada para comparação de simetria
...
%             filterType = 3; % Tipo III: h(n) antissimétrico, M ímpar
%         else
%             % Caso de Tipo IV (antissimétrico e M par)
%             filterType = 4; % Se necessário, adicione suporte para o Tipo IV
%         end
%     else
%         disp('Diagnóstico de simetria:');
%         disp('Diferença para simetria (h - flip(h)) =');
%         disp(h - flip(h));
%         disp('Diferença para antissimetria (h + flip(h)) =');
%         disp(h + flip(h));
%         error('A resposta ao impulso h(n) não é de fase linear!');
%     end
...
%             [Hr, w, P, L] = Hr_Type3(h);
%         case 4
%             % Caso seja implementado Hr_Type4, adicione a linha abaixo:
%             % [Hr, w, P, L] = Hr_Type4(h);
%             error('Tipo IV não é suportado nesta implementação.');
%     end
% end
